...
class BigCodeBenchLoader(DatasetLoader):
    ...
    def load(self, dataset_source: str, split: str = "test", **kwargs) -> List[CodeProblem]:
        """Load BigCodeBench dataset from JSONL file"""
        
        if not dataset_source.endswith('.jsonl'):
            raise ValueError(f"BigCodeBench loader expects .jsonl file, got: {dataset_source}")
        ...
        with open(dataset_source, 'r') as f:
            for idx, line in enumerate(f):
                ...
                # Use complete_prompt or code_prompt as the problem text
                problem_text = item.get('complete_prompt', item.get('code_prompt', ''))
                
                # Create problem
                problem = CodeProblem(
                    problem_id=item.get('task_id', f'BigCodeBench/{idx}').replace('/', '_'),
        ...
        print(f"Loaded {len(problems)} problems from BigCodeBench JSONL")
        return problems
...
class LoaderFactory:
    ...
    _loaders = {
        ...
        'bigcodebench': BigCodeBenchLoader,
    }
...
class CodeEvaluator:
    ...
    async def evaluate_problem(self, problem: CodeProblem) -> Dict[str, Any]:
        ...
        # Evaluate responses
        time_start = time.time()
        results = None #await self.evaluate_responses(problem, choices)
        strategies = None #self.apply_selection_strategies(results)
        time_end = time.time()
    ...
    async def evaluate_dataset(self) -> Dict[str, Any]:
        ...
        print("loaded problems for evaluation")
...
async def main():
    ...
    print("\n" + "="*80)
    print("CODE EVALUATION CONFIGURATION")
    ...
    print(f"OpenAI Client initialized with base URL: {config.base_url}")
