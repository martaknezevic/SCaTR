...
os.environ["WANDB_PROJECT"] = 'cactts_sft2'
os.environ['WANDB_API_KEY'] = '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'
...
def load_data(model_name: str, dataset_name: str, turn: int) -> List[Dict]:
    ...
    if dataset_name in ("humaneval", "kodcode"):
...
def evaluate_best_of_n(
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(all_prompts), batch_size), desc="Evaluating"):
            ...
            ### print top 5 tokens and their probabilities for the first item in the batch (for debugging) - decode the tokens
            # print('################################################')
            # topk = torch.topk(probs[0], k=5)
            # print("\nTop 5 predicted tokens for first item in batch:")
            # for token_id, prob in zip(topk.indices.cpu().tolist(), topk.values.cpu().tolist()):
            #     token_str = tokenizer.decode([token_id])
            #     print(f"  Token: '{token_str}' (id {token_id}), Probability: {prob:.4f}")
            # print('################################################')
    ...
    return accuracy, rollout_predictions, problem_results

# ── per-seed entry-point ─────────────────────────────────────────────────────

def process_seed(args_tuple):
    """Process a single (seed, turn) experiment."""
    seed, gpu_id, train_ds, test_ds, model_name, hf_model, turn, bs, bs_eval, grad_accumulation_steps, max_seq_len, eval_only = args_tuple
    ...
    try:
        ...
        output_dir = f"/efs/cactts/models/baselines/{model_name}_{train_ds}_{test_ds}/seed_{seed}_turn{turn}"
        ...
        accuracy, rollout_predictions, problem_results = evaluate_best_of_n(hf_model, model_dir, test_data, gpu_id, batch_size=bs_eval, max_seq_len=max_seq_len)
...
def main():
    ...
    for idx, seed in enumerate(seeds):
        for turn in turns:
            tasks.append(
                (seed, 0, args.train, args.test, args.model, args.hf_model, turn, args.bs, args.bs_eval, args.grad_accum, args.max_seq_len, args.eval_only)
            )
